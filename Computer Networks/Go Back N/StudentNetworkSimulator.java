...
integer that represents entity B
                                   ...
                                   double delay)
    {
        ...
        WindowSize = winsize;
        LimitSeqNo = winsize + 1;// Assumes GBN here !
        RxmtInterval = delay;
    }
    ...
    protected void aInput(Packet packet)
    {
        ...
        for(int i = 0; i < packet.getSack().length; i++){
            isACKed[sack[i]] = 1; // All the selectively acknowledged packets are now marked as ACKed
        }
    ...
    protected void aTimerInterrupt()
    {
        ...
        for(int i = sender.base; i < sender.nextSeqNo; i++){ //Go Back N is imposed here ! All unacknowledged packets are resent
            Packet packet = sender.buffer[i];
            System.out.println("[aTimerInterrupt] Resending packet : " + (packet.getSeqnum() % LimitSeqNo));
            toLayer3(A, packet);
            retransmittedPackets += 1; //Keeps track of the number of retransmitted packets
            isRetransmitted[packet.getSeqnum()] = 1; //To know which packets have been retransmitted
        }
    ...
    protected void bInput(Packet packet)
    {
        if(packet.getChecksum() != checksumPacket(packet.getSeqnum(), packet.getAcknum(), packet.getPayload())){ //To handle corrupt packets
        ...
        else if(packet.getSeqnum() > receiver.expectedSeqNo){ // To handle out-of-order packets since RWS = 1

            System.out.println("[bInput] Out-of-order packet received. Drop !");
        }
            ...
            int ackNo = packet.getSeqnum();
            int checksum = checksumACK(seqNo,ackNo);
            int expSeqNo = receiver.expectedSeqNo;
            int[] sack = new int[5];
            for(int i = 0; i < 5; i++){
                sack[i] = expSeqNo; // Selectively acknowledging the latest five data packets tha have been successfully received at the receiver.
                --expSeqNo;
            }
            Packet ack = new Packet(seqNo, ackNo, checksum, sack);
